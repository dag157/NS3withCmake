...
macro (build_lib libname source_files header_files libraries_to_link test_sources)
    ...
    #    execute_process(
    #            COMMAND  PYTHONPATH=${CMAKE_OUTPUT_DIRECTORY} ${Python2_EXECUTABLE} ${PROJECT_SOURCE_DIR}/bindings/python/ns3modulegen-modular.py ${PROJECT_SOURCE_DIR}/src/${libname}/bindings/modulegen__${arch}.py > ${CMAKE_OUTPUT_DIRECTORY}/ns/${libname}-module.cc
    #    )
    #endif()

    #Build pybindings if requested
    #remember to target bindings/python/ns
    if(${NS3_PYTHON})
        set(arch gcc_LP64)#ILP32)
        ...
        file(COPY ./bindings/${module_src} DESTINATION ${CMAKE_SOURCE_DIR}/build/src/${libname}/bindings/)
        file(COPY ./bindings/${module_hdr} DESTINATION ${CMAKE_SOURCE_DIR}/build/src/${libname}/bindings/)

        #set(modulegen_modular_command ${CMAKE_COMMAND} -E env PYTHONPATH=${CMAKE_OUTPUT_DIRECTORY} ${Python2_EXECUTABLE} ${CMAKE_SOURCE_DIR}/bindings/python/ns3modulegen-modular.py ${CMAKE_CURRENT_SOURCE_DIR} ${arch} ${libname} ./bindings/${module_src})
        set(modulegen_modular_command ${CMAKE_COMMAND} -E env PYTHONPATH=${CMAKE_OUTPUT_DIRECTORY} ${Python2_EXECUTABLE} ${CMAKE_SOURCE_DIR}/bindings/python/ns3modulegen-modular.py ${CMAKE_SOURCE_DIR} ${arch} ${libname} ./build/src/${libname}/bindings/${module_src})
        set(modulegen_arch_command ${CMAKE_COMMAND} -E env PYTHONPATH=${CMAKE_OUTPUT_DIRECTORY}/ ${Python2_EXECUTABLE} ./bindings/modulegen__${arch}.py 2> ./bindings/ns3modulegen.log)
        ...
        if(NOT TARGET ${libname})
            ...
            add_library(${libname} SHARED ${CMAKE_CURRENT_SOURCE_DIR}/bindings/${module_src} ${CMAKE_CURRENT_SOURCE_DIR}/bindings/${module_hdr})
            target_link_libraries(${libname} ${LIB_AS_NEEDED_PRE} ${ns3-libs} ${Python2_LIBRARIES} ${LIB_AS_NEEDED_POST})
